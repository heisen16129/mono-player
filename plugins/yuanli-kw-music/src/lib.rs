...
const PROVIDER_ID: &str = "mono-native-wasm-yuanli-kw";
const DEFAULT_QUALITY: &str = "320k";
const PROVIDER_NAME: &str = "元力KW";
...
fn metadata_response() -> Value {
    json!({
        ...
        "kind": "music",
        "author": "Mono",
        "description": "搜索并播放酷我音乐，支持在线歌曲解析和歌词获取。",
        "updatedAt": "2026-07-23",
        "capabilities": ["search", "play", "lyrics"],
        "highlights": ["支持在线播放", "返回多格式歌词", "可作为备用音源"],
        "permissions": ["network"],
        "configSchema": {
            "fields": [
                {
...
fn play_request(request: &Value) -> Value {
    ...
    let Some(id) = source_raw_field(track, &["id", "MUSICRID", "DC_TARGETID"]).map(|v| v.replace("MUSIC_", "")) else {
        return json!({"error":"Kuwo track has no playable id."});
    };
    host_get(
        &format!("http://musicapi.haitangw.net/music/kw.php?id={id}&level={quality}"),
        headers(&[
            ("Referer", "http://musicapi.haitangw.net/"),
...
fn parse_lyrics_response(request: &Value, body: &str) -> Value {
    ...
    if raw.is_none() {
        if let Some(id) = source_raw_field(request.get("track").unwrap_or(&Value::Null), &["id", "MUSICRID", "DC_TARGETID"])
            .map(|v| v.replace("MUSIC_", ""))
        {
            return host_get(
                &format!("http://m.kuwo.cn/newh5/singles/songinfoandlrc?musicId={id}&httpsStatus=1&reqId={id}"),
                headers(&[
                    ("Referer", "http://m.kuwo.cn/"),
                    ("User-Agent", browser_user_agent()),
                ]),
...
fn parse_lyrics_fallback_response(request: &Value, body: &str) -> Value {
...
fn host_response(request: &Value) -> Value {
    ...
    match original.get("action").and_then(Value::as_str) {
        ...
        Some("host_response")
            if original.pointer("/request/action").and_then(Value::as_str) == Some("lyrics") =>
        {
            parse_lyrics_fallback_response(original, body)
        }
