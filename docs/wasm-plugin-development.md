...
插件所有请求和响应都是 JSON。插件内部用 `request.action` 分发方法。

## 订阅格式

订阅地址可以直接是一个 `.wasm` URL，也可以返回 JSON。

最小推荐 JSON：

```json
{
  "name": "Mono WASM Plugins",
  "plugins": [
    {
      "url": "https://example.com/plugins/example.wasm"
    }
  ]
}
```
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `url` | `url` 和 `entry` 二选一 | `.wasm` 地址。用于下载插件并读取 metadata。 |
| `entry` | `url` 和 `entry` 二选一 | `.wasm` 地址。存在时作为实际运行入口。 |

注意：订阅里的 `id/name/version/author/kind/capabilities/permissions` 不是权威来源。当前安装和刷新 catalog 时，Rust 会读取 `.wasm` 的 `metadata`，以插件自己返回的字段为准。
...
| `kind` | 是 | 插件类型，只支持 `music` 或 `lyrics`。 |
| `author` | 否 | 作者。当前内置插件都有 `author: "Mono"`。 |
| `description` | 否 | 插件说明。当前 UI 暂未重点展示。 |
| `capabilities` | 是 | 能力列表，不能为空。只支持 `search`、`play`、`lyrics`。 |
...
| --- | --- | --- | --- |
| `metadata` | 无 | 添加订阅、更新订阅、安装、本地导入 | 返回插件声明字段。 |
...
| `host_response` | 取决于原 action | 插件返回 `hostRequest` 后 | 插件解析 Host HTTP 响应并返回最终业务结果。 |
...
| --- | --- | --- |
| `track` | object | 搜索结果的 `raw`，如果没有 `raw` 就是整个搜索 track。 |
| `format` | string 或 null | 希望的歌词格式。当前支持 `lrc`、`trans`、`yrc`、`qrc`、`krc`、`a2`。 |
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `rawLyrics` | `rawLyrics` 和 `lyricsUrl` 至少建议一个 | 原始歌词文本。也兼容 `rawLrc`、`rawLrcTxt`、`lyric`、`lyrics`、`lrc`。 |
| `lyricsUrl` | 否 | 歌词 URL。也兼容 `lyricUrl`、`lrcUrl`。如果没有 `rawLyrics`，Host 会尝试拉取这个 URL。 |
| `formats` | 否 | 可用格式列表。 |
| `defaultFormat` | 否 | 默认格式。 |
| `format` | 否 | 本次返回格式。缺失时使用 `defaultFormat`。 |
...
插件收到 `host_response` 后，应根据原始 `request.action` 解析 `response.body`，并返回最终的 `search`、`play` 或 `lyrics` 响应。
...
- 歌词插件或音乐插件的 `lyrics` 能返回 `rawLyrics` 或 `lyricsUrl`。
