...
const PROVIDER_ID: &str = "mono-wasm-aq-lyrics";
const PROVIDER_NAME: &str = "阿Q歌词";
const API_BASE: &str = "https://api.vkeys.cn/v2/music/tencent";
...
fn search_request(request: &Value) -> Value {
    ...
    json!({
        "hostRequest": {
            "method": "GET",
            "url": format!("{API_BASE}/search/song?word={}&page={page}&num={page_size}", url_encode(keyword)),
...
fn lyrics_request(request: &Value) -> Value {
    ...
    let query = if let Some(id) = id {
        ...
        format!("mid={}", url_encode(&mid))
    } else {
        return json!({ "error": "lyrics track missing id or mid" });
    };
    json!({
        "hostRequest": {
            "method": "GET",
            "url": format!("{API_BASE}/lyric?{query}"),
            "headers": default_headers(),
            "data": null
        }
    })
...
fn parse_search_response(request: &Value, body: &str) -> Value {
    ...
    let tracks = payload
        ...
        .map(|items| items.iter().map(normalize_search_track).collect::<Vec<_>>())
        .unwrap_or_default();
    let page_size = request
        .get("pageSize")
        .and_then(Value::as_u64)
...
fn normalize_search_track(item: &Value) -> Value {
    let id = item.get("id").and_then(value_to_string).unwrap_or_else(|| {
        item.get("mid")
            .and_then(value_to_string)
            .unwrap_or_default()
    });
    let mid = item.get("mid").and_then(Value::as_str).unwrap_or("");
    let title = item.get("song").and_then(Value::as_str).unwrap_or("");
    let artist = item.get("singer").and_then(Value::as_str).unwrap_or("");
    ...
    json!({
        "id": if id.is_empty() { mid } else { id.as_str() },
